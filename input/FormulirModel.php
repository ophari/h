...
$id_jadwal = rand(100000000, 999999999); // assuming 9 digits is long enough for uniqueness
// check if id_jadwal already exists in database
$check_id_jadwal_query = "SELECT * FROM formulir WHERE id_jadwal='$id_jadwal'";
$result = mysqli_query($this->conn, $check_id_jadwal_query);
$data = mysqli_fetch_assoc($result);

// if id_jadwal already exists, generate a new one until id_jadwal is unique
while ($data) {
    $id_jadwal = rand(100000000, 999999999);
    $check_id_jadwal_query = "SELECT * FROM formulir WHERE id_jadwal='$id_jadwal'";
    $result = mysqli_query($this->conn, $check_id_jadwal_query);
    $data = mysqli_fetch_assoc($result);
}
      ...
      '$this->no_telp_keluarga', '$this->foto', '$id_jadwal', '$this->tanggal_keberangkatan', '$this->maskapai', '$this->tanggal_pulang', '$this->timestamp', '$this->status')";
